...
(function($) {
	$.fn.isInViewport = function isInViewport(options) {
		...
		var normalJsThisObj = this.get(0),
			...
			endOfPage = ($(window).scrollTop() === ($(document).height() - winheight)) ? true : false,
			...
			isVisibleFlag = false,
			halfOfTolerance = settings.tolerance/2;
		...
		if (settings.tolerance) {
			...
			if( ( top >= -halfOfTolerance && top <= (winheight+halfOfTolerance) ) ||
				( bottom >= -halfOfTolerance && bottom <= (winheight+halfOfTolerance) ) )
				isVisibleFlag = true;
			else
				isVisibleFlag = false;
			...
			if ( (top >= 0 && top <= winheight) ||
				(bottom >= 0 && bottom <= winheight) )//$(window).height()
				isVisibleFlag = true;
			else
				isVisibleFlag = false;
		}
